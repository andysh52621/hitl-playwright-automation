...
def test_cascading_dropdown_behavior(page, test_user, reporter, ado_runner, request):
    ...
    validate_multicolumn_autocomplete_dropdown_task(page, test_meta, test_user, reporter, steps)


@allure.step("Validate and save cascading dropdown task")
def validate_multicolumn_autocomplete_dropdown_task(page, test_meta, test_user, reporter: DBReporter, step_logger: StepLogger):
    task_page = TaskManagerListPage(page)
    task_page.begin_validate_autocomplete_dropdown(test_meta, test_user, reporter, step_logger)
